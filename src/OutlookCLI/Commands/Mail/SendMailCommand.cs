...
{
    public SendMailCommand() : base("send", "Send a new email. Use --no-confirm to skip confirmation in automated workflows.")
    {
    ...
    private void Execute(string[] to, string[]? cc, string subject, string? body, FileInfo? bodyFile, bool isHtml, FileInfo? signatureFile, FileInfo[]? attachments)
    {
        ...
        {
            service.Initialize();
            service.SendMail(to, cc, subject, emailBody, isHtml, attachmentPaths.ToArray());
